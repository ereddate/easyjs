(function(global, undefined) {
	...
	var doc = global.document,
		...
		stringify = function(str) {
			...
			var type = fixtype(str).toLowerCase();
			switch (type) {
				case 'string':
		...
		isEmpty = function(v) {
			...
			return typeof v == fixUndefined || v == null || typeof v == "string" && v.trim() == "" || isArray(v) && v.length == 0 || typeof v == "object" && a(v) ? true : false;
		},
		isArray = function(v) {
		...
		merge = function(arr, inarr, func) {
			if (typeof inarr == fixUndefined) return arr;
			each(arr, function(i, value) {
				inarr.push(func ? func(value) : value);
			});
			return inarr;
		},
		fix = function(name) {
	...
	var obj = function(id, deps) {
		mix(this, {
			id: id,
			uri: id,
			dependencies: merge(deps, [], function(value){
				return fix(value).url || "";
			})
		});
		return this;
	};
	...
	obj.define = function(/*id, dependencies, factory*/) {
		...
		dependencies = dependencies || merge((code && parseDependencies(code) || []), [], function(value){
			return fix(value).url;
		});
					...
					if (dependencies.length > 0) {
						easyjs.use(dependencies.join(' '), function() {
							var list = dependencies.slice(),
								codes = merge(list, [], function(value){
									return cache[value].factory;
								});
							list.push(id);
							codes.push(code);
	...
	if (loadeasyjs) {
		...
		if (config) {
			easyjs.use(config, function() {
				...
				if (loads && loads.length > 0) easyjs.use(loads.join(' '));
			});
		}
	}
